...
def fetch_role(m):
  ...
  for i, n in enumerate([985750157607985153, 987232675221868604, 987232721673814016]):
...
async def on_message(message):
  if message.channel.id == 985752337710723163:
    ...
    if not wrong_counting and entry_count == current_count + 1:
      ...
      f = open('number.txt', 'w')
      f.writelines([str(current_count) + "\n", ",".join(correct_lst), ",".join(error_lst)])
      f.close()
    else:
  ...
  if message.content.startswith("$score"):
    ...
    error_lst = f.readline().split(",")
    f.close()
    correct_lst[2] = correct_lst[2][:-1]
    output = "This command is used for debugging and testing, not to be actually implemented"
    for i, name in enumerate(["Team Kusanali", "Team Itto", "Team Kuki"]):
